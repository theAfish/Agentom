...
def _format_http_exception(e: Exception) -> str:
    ...
    if resp is not None:
        ...
        try:
            ...
            json_body = None
        if json_body:
            parts.append(f"response_json={json_body}")
...
class CypherGenerator:
    ...
    def __init__(self) -> None:
        try:
            ...
            elif settings.LLM_PROVIDER == "deepseek":  # deepseek
                llm = OpenAIModel(
    ...
    async def generate(self, natural_language_query: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"  [CypherGenerator] Error: {_format_http_exception(e)}"
            )
...
def create_rag_orchestrator(
    ...
    try:
        ...
        return Agent(
            model=llm,
            system_prompt=system_prompt or RAG_ORCHESTRATOR_RETRIVAL_PROMPT,
            tools=tools,
            model_settings=model_settings,
        )  # type: ignore
    except Exception as e:
